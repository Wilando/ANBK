...
--
-- Host: localhost:3306
-- Generation Time: Oct 11, 2024 at 03:54 AM
...
CREATE TABLE `tr_iku_opd` (
  ...
  `target_opd` decimal(10,2) DEFAULT NULL,
  `realisasi_opd` decimal(10,2) DEFAULT NULL,
  `capaian_opd` decimal(5,2) DEFAULT NULL,
  `keterangan` text,
  `id_program` int DEFAULT NULL,
  `id_kegiatan` int DEFAULT NULL,
  `id_sub_kegiatan` int DEFAULT NULL,
...
CREATE TABLE tr_rencana_kerja_pd (
    id_rencana_kerja SERIAL PRIMARY KEY,
    id_unker INT,  -- Relasi ke master_unker
    id_urusan INT,  -- Relasi ke master_urusan
    id_sub_urusan INT,  -- Relasi ke master_sub_urusan
    id_program INT,  -- Relasi ke master_program
    id_kegiatan INT,  -- Relasi ke master_kegiatan
    id_sub_kegiatan INT,  -- Relasi ke master_sub_kegiatan
    id_indikator INT,  -- Relasi ke tr_indikator
    target_akhir_rpjmd INT,  -- Kolom untuk Target Akhir Periode RPJMD
    realisasi_thn_1 NUMERIC(20,2),  -- Kolom untuk Realisasi RKPD Tahun 2022
    prakiraan_thn_1 NUMERIC(20,2),  -- Kolom untuk Prakiraan Capaian Target RKPD 2023
    target_thn_n INT,  -- Kolom untuk Target 2024
    pagu_indikatif_thn_berjalan NUMERIC(20,2),  -- Kolom untuk Pagu Indikatif 2024
    lokasi VARCHAR(255),  -- Kolom untuk Lokasi
    sumber_dana VARCHAR(255),  -- Kolom untuk Sumber Dana
    prioritas_nasional BOOLEAN,  -- Kolom untuk Prioritas Nasional
    prioritas_daerah BOOLEAN,  -- Kolom untuk Prioritas Daerah
    kelompok_sasaran VARCHAR(255),  -- Kolom untuk Kelompok Sasaran
    pagu_indikatif_thn_n NUMERIC(20,2),  -- Kolom untuk Pagu Indikatif 2025
    perangkat_penanggung_jawab VARCHAR(255),  -- Kolom untuk Perangkat Daerah Penanggung Jawab
    is_active BOOLEAN DEFAULT TRUE,  -- Status aktif/tidak
    created_by INT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,  -- Set nilai default waktu pembuatan
    updated_by INT NULL,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,  -- Set nilai default saat pembaruan
    FOREIGN KEY (id_unker) REFERENCES master_unker(id)
        ON UPDATE CASCADE
        ON DELETE RESTRICT,
    FOREIGN KEY (id_urusan) REFERENCES master_urusan(id_urusan)
        ON UPDATE CASCADE
        ON DELETE RESTRICT,
    FOREIGN KEY (id_sub_urusan) REFERENCES master_sub_urusan(id_sub_urusan)
        ON UPDATE CASCADE
        ON DELETE RESTRICT,
    FOREIGN KEY (id_program) REFERENCES master_program(id_program)
        ON UPDATE CASCADE
        ON DELETE RESTRICT,
    FOREIGN KEY (id_kegiatan) REFERENCES master_kegiatan(id_kegiatan)
        ON UPDATE CASCADE
        ON DELETE RESTRICT,
    FOREIGN KEY (id_sub_kegiatan) REFERENCES master_sub_kegiatan(id_sub_kegiatan)
        ON UPDATE CASCADE
        ON DELETE RESTRICT
);
