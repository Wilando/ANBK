...
class KegiatanController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->titleData = 'Kegiatan';
        $this->columnRef = 'nama_kegiatan';
        $this->logName = 'kegiatan';
        $this->defaultRedirect = 'app/kegiatan';
        $this->nameTable = (new Kegiatan())->getTable();
        $this->primaryKey = (new Kegiatan())->getKeyName();
    ...
    {
        $data = $this->model::with(["program", "kegiatan_detail"]);
        ...
        $data = $data->where(function ($query) use ($created_by, $mulai, $selesai, $active, $program) {


            if ($created_by != '') {
                if (is_array($created_by)) {
                    $query->whereIn('master_kegiatan.created_by', $created_by);
                } else {
                    $query->where('master_kegiatan.created_by', $created_by);
                }
            }
            if ($mulai && $selesai) {
                $query->whereBetween('master_kegiatan.created_at', [$mulai, $selesai]);
            }

            if ($active != '') {
                $query->where('master_kegiatan.is_active', $active);
            }
            if ($program != '') {
                $query->where('master_kegiatan.id_program', $program);
            }
        });
        ...
        return DataTables::eloquent($data)
            ...
            ->addColumn('hashId', function ($row) {
                return encodeId($row->id_kegiatan);
            })
    ...
    public function store(Request $request)
    {
        ...
        $rule = [
            ...
            'kode_rekening' => 'required|unique:master_kegiatan',
            'program' => 'required|exists:master_program,id_program'
        ];
    ...
    public function show($id)
    {
        ...
        $listData = $this->model::view()->where('id_kegiatan', decodeId($id))->first();
    ...
    public function update(Request $request, $id)
    {
        ...
        $masterview = $this->model::view()->where('id_kegiatan', $id)->first();
        $rule = [
            'kode_rekening' => 'required|unique:master_kegiatan,kode_rekening,' . $id . ',id_kegiatan',
            'nama_kegiatan' => 'required',
            'program' => 'required|exists:master_program,id_program'
        ];
    ...
    {
        $master = $this->model::findOrfail(decodeId($id));
        $cekRelasi = SubKegiatan::where('id_kegiatan', $master->id_kegiatan)->exists() || KegiatanDetail::where('id_kegiatan', $master->id_kegiatan)->exists();
        if ($cekRelasi) {
            $res['message'] = 'Tidak dapat menghapus, Kegiatan sudah terkait pada data lain';
            $res['success'] = false;
    ...
    public function bulkDelete(Request $request)
    {
        ...
        foreach ($reqData["ids"] as $id) {
            $cekRelasi = SubKegiatan::where('id_kegiatan', $id)->exists();
            if ($cekRelasi) {
    ...
    public function unkerDelete(Request $request)
    {
        ...
        if (count($selected_ids) > 0) {
            KegiatanDetail::whereIn('id_kegiatan_detail', $selected_ids)->delete();
    ...
    public function remote(Request $request)
    {
        ...
        $dataRemote = $dataRemote
            ->with(
                'program.subUrusan.urusan:id_urusan,kode_rekening,nama_urusan',
                'program.subUrusan:id_sub_urusan,id_urusan,kode_rekening,nama_sub_urusan',
                'program:id_program,id_sub_urusan,kode_rekening,nama_program'
            )
            ->select(
                'id_kegiatan as id',
                'nama_kegiatan as text',
                'kode_rekening',
                'id_program'
            )
    ...
    {
        $processData = KegiatanDetail::create($reqData);
    ...
    public function inputDetail(Request $request)
    {
        ...
        $validator->after(function ($validator) use ($request) {
            ...
            $exists = DB::table('master_kegiatan_detail') // Ganti 'nama_tabel' dengan tabel yang sesuai
                ->where('id_unker', $request->input('unit_kerja'))
                ->where('id_kegiatan', $request->input('kegiatan'))
                ->exists();
    ...
    private function _hapusDetail($master)
    {
        ...
        if ($deleted) {
            ...
            activity()
                ...
                ->withProperties($properties)
                ->event('deleted')
                ->log('Hapus Unit Kerja Pada Kegiatan');
        }
    ...
    {
        $master = KegiatanDetail::findOrfail(decodeId($id));
